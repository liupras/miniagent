...
@pytest.mark.parametrize(
    "name, phase, allowed, forbidden",
    [
        (
            "virtual_court_investigation_judge",
            "调查",
            ("ASK", "COMPLETE", "HANDOFF", "allowed_actions", "allowed_targets"),
            ("CONTINUE", "NO_ACTION", "EXPLAIN_LAW", "intellectual_property_law_search"),
        ),
        (
            "virtual_court_debate_judge",
            "辩论",
            ("CONTINUE", "COMPLETE", "HANDOFF", "allowed_actions", "current_issue"),
            ("ASK", "NO_ACTION", "EXPLAIN_LAW", "intellectual_property_law_search"),
        ),
    ],
)
def test_flow_agent_prompts_only_contain_phase_rules(name, phase, allowed, forbidden):
...
def test_all_three_agents_load_and_only_law_check_has_law_tool():
