...
> [!IMPORTANT]
> MiniAgent 当前处于开发阶段，数据库结构和接口会不断调整。升级前请备份 `backend/db` 与 `backend/files`。
...
![管理后台](assert/management_cn.gif)

![用户工作台](assert/workplace_cn.gif)
...
- DB init — app/infra/db/initializer.py 创建 SQLite 表并从 app/infra/db/seed/ 加载种子 JSON
- ServiceContainer — 构建异步 SQLAlchemy 引擎、所有存储库和长期运行的服务
- Domain plugins — 从数据库加载领域行并通过动态导入注册知识库处理器

所有共享资源都位于 request.app.state.container 中。路由通过 Depends(get_container) 获取它。
...
- 修改智能体、知识库、工具或模型配置时，后台会自动刷新缓存，如果缓存系统继续使用旧配置，可以在 `management` 中手动刷新缓存。
